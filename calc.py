...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.maxsize(330, 430)
        self.minsize(330, 430)
...
#================== row 4
        self.button_sqr = ctk.CTkButton(self.frame, text = "^2", corner_radius = 10, font=("Arial", 15), border_width = 2, command = lambda: [self.insert("^2"), self.insert_list("**2")])
        self.button_sqr.grid(row = 4, column = 0, padx = 10, pady = 10, sticky = "nsew", columnspan = 1, rowspan = 1)

        self.button_sqrt = ctk.CTkButton(self.frame, text = "√", corner_radius = 10, font=("Arial", 15), border_width = 2, command = lambda: [self.insert("√"), self.insert_list("**0.5")])
